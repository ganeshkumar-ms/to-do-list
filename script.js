...
function popTask(position) {
  ...
  const index = tasks.findIndex(task => task.currentPosition === position);
  
  if (index > -1) {
    const taskItem = document.querySelector(`.task-${position}`);
    taskItem.style.animation = "taskSlideOut 0.5s ease-out forwards";

    setTimeout(() => {
      container.removeChild(taskItem);
      tasks.splice(index, 1); // Remove the task from the array
      updatePositions(); // Update the positions of remaining tasks
...
function updatePositions() {
  tasks.forEach((task, index) => {
    task.currentPosition = index; // Reassign positions
  });
  currentPosition = tasks.length; // Update current position
}
